...
- 然后，在 MTD 建设完成后，我们为了更加方便、快速的接入外卖的相关业务，定制了外卖主题的组件库 Roo，提升了组件库的业务功能性和易用性。外卖相关的业务项目，在接入 Roo 后直接使用，无需再进行主题的定制与调整。
...
- 样式一致性的保证。主要通过定义全局性的主题变量，供所有组件使用，变量范围涉及到品牌色、灰度、字体尺寸、间距以及边线等。同时，组件库提供了自定义主题变量的接口，可以重置相关变量的值，实现一键换肤（下文图 E），UI 风格一致性调整。另外，使用“内部样式 < 主题 < 扩展样式”的样式优先级覆盖策略，保证了样式部分的定制能力（在下文“定制化能力分级设计”章节中详细介绍）。
