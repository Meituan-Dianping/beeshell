...
组件库的架构整体分成四层，包括业务层、组件库体系、RN 层、系统层。

组件库体系包含完善的测试体系，丰富的代码示例，使用 TypeScript（以下简称 TS）语言进行开发，充分利用 TS 的类型定义与检查，针对每个组件都有详细的文档说明。最终，提供一套通用、易用、稳定、扩展能力强的高质量组件。
...
样式的一致性，还涉及到图标、布局等相关内容，这里不做详细介绍，如有需要可以参考 MATERIAL DESIGN。
