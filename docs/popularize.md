...
组件库的架构整体分成四层，包括业务层、组件库体系、RN 层、系统层。本文只介绍第二层与第三层：

**第二层，组件库体系**

组件库由一个项目，演变成三个项目（版本），形成一个组件库体系。具体包含：MTD 公司通用版本、Roo 外卖定制版本和 beeshell 开源版本。三个版本之间通过 Git Fork 建立依赖关系，源码级的依赖，既实现了代码复用，又更加利于组件的定制化扩展。
...
三个版本的组件库架构设计一致，本文只详细介绍 beeshell。组件库使用了分层的架构风格，分成了接口层、组件层与工具层。
...
另外，组件库体系包含完善的测试体系，丰富的代码示例，使用 TypeScript（以下简称 TS）语言进行开发，充分利用 TS 的类型定义与检查，针对每个组件都有详细的文档说明。最终，提供一套通用、易用、稳定、扩展能力强的高质量组件。
