...
组件库由 1.0 的一个项目，演变成 2.0 的三个项目（版本），形成了一个组件库体系。具体包含：MTD 公司通用版本、Roo 外卖定制版本和 beeshell 开源版本。三个版本之间通过 Git Fork 建立依赖关系，源码级的依赖，既实现了代码复用，又更加利于组件的定制化扩展。

beeshell 2.0 这样架构的原因，是因为我们要开发全公司共用的组件库，大家公司级组件库的特性要求是：既要通用、稳定、扩展性强，又要易用、功能强大。然而，对一个组件来说，如果通用性、扩展性强，则功能性、易用性势必较差。如何将两个互相矛盾的方面整合？
...
下文详细介绍下 beeshell 1.0 与 beeshell 2.0 以及 MTD、Roo 的关系，让大家有更加清晰的了解：
...
beeshell 基于 Roo Theme 输出的主题变量，通过样式和动效两个方面，保证了 UI 一致性。

- 样式一致性的保证。主要通过全局性的主题变量，用于组件的样式部分定义。主题变量范围，涉及品牌色、灰度、字体尺寸、间距以及边线等各方各面，为组件以及组件之间样式一致性，提供了全面的保障。同时，组件库提供了自定义主题变量的接口，可以重置相关变量的值，实现一键换肤（下文图 E），UI 风格进行一致性调整。另外，使用“内部样式 < 主题 < 扩展样式”的样式优先级覆盖策略，保证了样式部分的定制能力（在下文“定制化能力分级设计”章节中详细介绍）。
