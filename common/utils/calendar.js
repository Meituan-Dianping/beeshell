...
function getCalendar(date) {
    ...
    for (let i = 1; i < ROWS_OF_CALENDAR; i++) {
    ...
    for (let i = ret.length; i < ROWS_OF_CALENDAR; i++) {
        let sunday = firstDate.clone().day(0).day(i * DAYS_OF_A_WEEK);
        ret.push(getAWeek(sunday));
    }
...
function ViewDate(options) {
    ...
    // this.targetMonth = typeof options.targetMonth === 'boolean' ?
    //         options.targetMonth : true;

    // this.disabled = typeof options.disabled === 'boolean' ?
